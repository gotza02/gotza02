...
function Set-DefenderRealtime($Enable) {
    ...
    try {
        ...
        Write-Warning "Tamper Protection อาจเปิดใช้งานอยู่"
    }
}
...
function Update-AppsAndDrivers {
    ...
    if (Test-Path 'C:\Drivers') {
        Get-ChildItem 'C:\Drivers' -Recurse -Filter *.inf | ForEach-Object {
...
function Cleanup-TempAndPrefetch {
    ...
    cleanmgr /sagerun:1
    'C:\Windows\Prefetch', "$env:TEMP", "$env:SystemRoot\Temp" | ForEach-Object {
        if (Test-Path $_) { Get-ChildItem $_ -Recurse -Force | Remove-Item -Force -Recurse -ErrorAction SilentlyContinue }
    }
}
...
function Windows-Debloat {
    ...
    Read-Host "กำลังจะรัน $url กด Enter เพื่อดำเนินการ หรือ Ctrl+C ยกเลิก"
    irm $url | iex
}
