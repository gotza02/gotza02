...
sc stop "WSearch"
:: Add more unnecessary services for Windows 11 - Be cautious disabling services, research impact first.
:: Consider adding options to disable these instead of hardcoding.
:: sc config "WerSvc" start= disabled  :: Windows Error Reporting Service - Consider impact on error reporting
:: sc stop "WerSvc"
:: sc config "dmwappushservice" start= disabled :: dmwappushservice (Device Management Wireless Application Protocol (WAP) Push service) - Consider if device management is used.
...
ipconfig /flushdns
:: Get the interface name dynamically for DNS setting
for /f "tokens=3*" %%i in ('netsh int show interface ^| findstr /i "Wi-Fi\|Ethernet"') do set interfaceName=%%j
if not defined interfaceName (
    echo No "Wi-Fi" or "Ethernet" interface found. DNS Optimization may not apply.
    pause
    goto optimize_internet
)
echo Setting DNS for interface: "!interfaceName!"
netsh int ip set dns name="!interfaceName!" source=static address=8.8.8.8 registerdns=primary
netsh int ip add dns name="!interfaceName!" addr=8.8.4.4 index=2
...
for /f "tokens=3*" %%i in ('netsh int show interface ^| findstr /i "Connected"') do (
    ...
    powershell -Command "& { Get-NetAdapterAdvancedProperty -Name '%%j' -DisplayName 'Flow Control' | Set-NetAdapterAdvancedProperty -RegistryValue 0 }"
    powershell -Command "& { Get-NetAdapterAdvancedProperty -Name '%%j' -DisplayName 'Interrupt Moderation' | Set-NetAdapterAdvancedProperty -RegistryValue 0 }"
    powershell -Command "& { Get-NetAdapterAdvancedProperty -Name '%%j' -DisplayName 'Priority & VLAN' | Set-NetAdapterAdvancedProperty -RegistryValue 3 }"
    powershell -Command "& { Get-NetAdapterAdvancedProperty -Name '%%j' -DisplayName 'Speed & Duplex' | Set-NetAdapterAdvancedProperty -RegistryValue 'Auto Negotiation' }" :: Set Speed & Duplex to Auto Negotiation for better compatibility
)
...
if "%update_choice%"=="4" goto config_auto_updates
...
:: Add browser cache clearing - Chrome example - Consider other browsers and user profiles.
...
echo 6. Manage Storage Sense (Windows 11)
...
if "%disk_choice%"=="6" goto manage_storage_sense
...
:: Use the dynamic interface name variable from DNS optimization
if not defined interfaceName (
    echo No network interface name defined. Cannot set static DNS.
    pause
    goto network_optimization
)
netsh interface ip set dns name="!interfaceName!" static %primary_dns% primary
netsh interface ip add dns name="!interfaceName!" addr=%secondary_dns% index=2
...
call :modify_registry "HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\FocusAssist" "State" "REG_DWORD" "1" :: Corrected Registry Key "State"
...
call :modify_registry "HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\FocusAssist" "State" "REG_DWORD" "3" :: Corrected Registry Key "State"
...
call :modify_registry "HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\FocusAssist" "State" "REG_DWORD" "%focus_val%" :: Corrected Registry Key "State"
...
echo Script developed by Versatile AI Assistant
echo Version 4.1
pause
exit
