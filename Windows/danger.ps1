...
.NOTES
    Author: AI Assistant (Adding Auto Cache Clear Task to the final script)
...
if (-Not ([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
    Write-Error "!!! FATAL ERROR: SCRIPT MUST BE RUN AS ADMINISTRATOR. EXECUTION HALTED. !!!"; Read-Host "Press Enter to exit..."; Exit
}
...
$TotalPhases = 13 # Increased total phases for Startup Cleanup Task Setup
$CurrentPhase = 0
...
# ... (TP Disable code remains the same) ...
...
# ... (Power settings code remains the same as v5) ...
...
# ... (Network settings code remains the same) ...
...
# ... (Pagefile code remains the same as v5) ...
...
# ... (Desktop Icons code remains the same as v5) ...
...
# --- Section 6: Create Startup Cache/Temp Cleanup Task --- NEW PHASE ---
$CurrentPhase = 6
...
Write-Host "Starting automatic cache and temp file cleanup..."
...
Function Clear-FolderContents {
    ...
    if (Test-Path \$FolderPath) {
        ...
        # Force removal, continue on errors (locked files)
        ...
        # Write-Host "- Folder not found: \$FolderPath" # Optional: reduce noise
    }
}

# --- General Temp Files ---
Write-Host "Clearing general temp locations..."
Clear-FolderContents -FolderPath "C:\Windows\Temp" # System Temp
# User Temp (Need to iterate through actual user profiles when run as SYSTEM)
Get-ChildItem -Path "C:\Users" -Directory | Where-Object { \$_.Name -notin @('Default', 'Public', 'All Users') -and (Test-Path "\$($_.FullName)\AppData\Local\Temp") } | ForEach-Object {
    Clear-FolderContents -FolderPath "\$($_.FullName)\AppData\Local\Temp"
}
...
# --- Browser Cache Cleanup (Iterate through user profiles) ---
Write-Host "Clearing browser cache folders (Chrome, Edge, Firefox, Brave)..."
Get-ChildItem -Path "C:\Users" -Directory | Where-Object { \$_.Name -notin @('Default', 'Public', 'All Users') -and (Test-Path "\$($_.FullName)\AppData\Local") } | ForEach-Object {
    ...
    Write-Host "- Checking profile: \$UserProfilePath"

    # Google Chrome
    Get-ChildItem -Path "\$UserProfilePath\AppData\Local\Google\Chrome\User Data" -Directory -Filter "Default" -ErrorAction SilentlyContinue | ForEach-Object {
        Clear-FolderContents -FolderPath "\$($_.FullName)\Cache"
        Clear-FolderContents -FolderPath "\$($_.FullName)\Code Cache"
        Clear-FolderContents -FolderPath "\$($_.FullName)\GPUCache"
    }
    # Iterate through Profile * folders too
    Get-ChildItem -Path "\$UserProfilePath\AppData\Local\Google\Chrome\User Data" -Directory -Filter "Profile *" -ErrorAction SilentlyContinue | ForEach-Object {
        Clear-FolderContents -FolderPath "\$($_.FullName)\Cache"
        Clear-FolderContents -FolderPath "\$($_.FullName)\Code Cache"
        Clear-FolderContents -FolderPath "\$($_.FullName)\GPUCache"
    }
    ...
    # Microsoft Edge
    Get-ChildItem -Path "\$UserProfilePath\AppData\Local\Microsoft\Edge\User Data" -Directory -Filter "Default" -ErrorAction SilentlyContinue | ForEach-Object {
        Clear-FolderContents -FolderPath "\$($_.FullName)\Cache"
        Clear-FolderContents -FolderPath "\$($_.FullName)\Code Cache"
        Clear-FolderContents -FolderPath "\$($_.FullName)\GPUCache"
    }
    Get-ChildItem -Path "\$UserProfilePath\AppData\Local\Microsoft\Edge\User Data" -Directory -Filter "Profile *" -ErrorAction SilentlyContinue | ForEach-Object {
        Clear-FolderContents -FolderPath "\$($_.FullName)\Cache"
        Clear-FolderContents -FolderPath "\$($_.FullName)\Code Cache"
        Clear-FolderContents -FolderPath "\$($_.FullName)\GPUCache"
    }
    ...
    # Mozilla Firefox
    Get-ChildItem -Path "\$UserProfilePath\AppData\Local\Mozilla\Firefox\Profiles" -Directory -Filter "*.default*" -ErrorAction SilentlyContinue | ForEach-Object {
        Clear-FolderContents -FolderPath "\$($_.FullName)\cache2\entries" # Main cache files
        Clear-FolderContents -FolderPath "\$($_.FullName)\startupCache"
        # Optionally clear offline cache: Clear-FolderContents -FolderPath "\$($_.FullName)\OfflineCache"
    }

     # Brave Browser
    Get-ChildItem -Path "\$UserProfilePath\AppData\Local\BraveSoftware\Brave-Browser\User Data" -Directory -Filter "Default" -ErrorAction SilentlyContinue | ForEach-Object {
        Clear-FolderContents -FolderPath "\$($_.FullName)\Cache"
        Clear-FolderContents -FolderPath "\$($_.FullName)\Code Cache"
        Clear-FolderContents -FolderPath "\$($_.FullName)\GPUCache"
    }
     Get-ChildItem -Path "\$UserProfilePath\AppData\Local\BraveSoftware\Brave-Browser\User Data" -Directory -Filter "Profile *" -ErrorAction SilentlyContinue | ForEach-Object {
        Clear-FolderContents -FolderPath "\$($_.FullName)\Cache"
        Clear-FolderContents -FolderPath "\$($_.FullName)\Code Cache"
        Clear-FolderContents -FolderPath "\$($_.FullName)\GPUCache"
    }

    # Add other browser cache locations here if needed

}

Write-Host "Automatic cache and temp file cleanup finished."

# Optional: Add a short pause if run visibly to see output
# Start-Sleep -Seconds 10
"@ # End of heredoc for script content

# 6.2 Save the cleanup script to the specified path
try {
    Write-Host "- Saving cleanup script to: $StartupCleanupScriptPath"
    Set-Content -Path $StartupCleanupScriptPath -Value $CleanupScriptContent -Force -Encoding UTF8
    Write-Host "  - Cleanup script saved successfully." -ForegroundColor Green
} catch {
    Write-Warning "- FAILED to save cleanup script to '$StartupCleanupScriptPath': $($_.Exception.Message)"
    Write-Warning "- Startup cleanup task creation will be skipped."
    # Skip Task Registration if script saving failed
    goto SkipTaskRegistration # Use goto to jump past registration
}

# 6.3 Register the Scheduled Task
try {
    Write-Host "- Registering Scheduled Task 'AutoCacheCleanup' to run at startup..."
    $Action = New-ScheduledTaskAction -Execute "powershell.exe" -Argument "-NoProfile -ExecutionPolicy Bypass -File `"$StartupCleanupScriptPath`"" # Ensure path is quoted
    $Trigger = New-ScheduledTaskTrigger -AtStartup
    $Principal = New-ScheduledTaskPrincipal -UserId "SYSTEM" -LogonType ServiceAccount -RunLevel Highest
    $Settings = New-ScheduledTaskSettingsSet -AllowStartIfOnBatteries -DontStopIfGoingOnBatteries -StartWhenAvailable -Hidden -ExecutionTimeLimit (New-TimeSpan -Minutes 15) # Give it more time
    Register-ScheduledTask -TaskName "AutoCacheCleanup" -Action $Action -Trigger $Trigger -Principal $Principal -Settings $Settings -Description "Automatically clears browser caches and temp files on startup (Created by AIO Script)" -Force -ErrorAction Stop
    Write-Host "  - Scheduled Task 'AutoCacheCleanup' registered successfully." -ForegroundColor Green
} catch {
     Write-Warning "- FAILED to register Scheduled Task 'AutoCacheCleanup': $($_.Exception.Message)"
}

:SkipTaskRegistration # Label for goto jump

Start-Sleep -Seconds 3


# --- Section 7: Aggressive RAM Management --- NOW PHASE 7 ---
$CurrentPhase = 7
...
# --- Section 8: Maximum Effort Disable Windows Defender --- NOW PHASE 8 ---
$CurrentPhase = 8
...
# ... (Hardware check code remains the same) ...
...
# --- Section 11: Attempt Windows Update (General OS & Other Drivers) --- NOW PHASE 11 ---
$CurrentPhase = 11
...
# ... (Windows Update code remains the same) ...
...
# --- Section 12: Final Cleanup (Includes fstrim) --- NOW PHASE 12 ---
$CurrentPhase = 12
...
# --- Final Phase Update & Auto-Restart --- NOW PHASE 13 ---
$CurrentPhase = 13
...
Write-Warning "***                                                                                                  ***"
Write-Warning "*** POST-RESTART: Startup cleanup task will run. Check HW Drivers. Monitor Temps/Stability.          ***"
